About
=====

The software is based on the Analytic Element Method (AEM) and is designed to simulate groundwater flow and transport in discrete fracture networks (DFNs). The software is written in Python and is open-source.

Contributors
------------
The code is written by and maintained by Dr. Erik Toller.

Acknowledgements
----------------
The development of the original code for AnDFN was funded by BeFo (Stifftelsen Bergteknisk Forskning) grant number 529.

Publications
------------
The software is based on the following publications:

Otto D.L. Strack, Erik A.L. Toller, An analytic element model for flow in fractured impermeable rock, *Journal of Hydrology*, 2024, 131983, ISSN 0022-1694, https://doi.org/10.1016/j.jhydrol.2024.131983.

.. toctree::
    :maxdepth: 1

    release_notes
